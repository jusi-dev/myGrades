...
export default function SubjectDetails(subject: any) {
    ...
    return (
        <div className="p-6">
            <div className="mb-6">
                <h1 className="text-4xl font-semibold text-pink-600">{subject.subject.subject_name}</h1>
            ...
            <div className="flex flex-col gap-5">
                <Dialog>
                    ...
                    <DialogContent>
                        ...
                        <DialogDescription className="flex flex-col gap-y-2">
                            <p className="mb-2 text-gray-700">Fülle alle Angaben aus, um eine Note hinzuzufügen</p>
                            <p className="text-pink-600 text-left">Notenbezeichnung <span className="font-light text-xs text-pink-500">(zB. LB 1)</span></p>
                            <Input type="text" value={gradeName} onChange={(e) => setGradeName(e.target.value)} placeholder="LB 1" />
                            <p className="text-pink-600 text-left">Note <span className="font-light text-xs text-pink-500">(zB. 5,5)</span></p>
                            <Input type="number" step={"0.01"} value={gradeValue} onChange={(e) => setGradeValue(parseFloat(e.target.value))} placeholder="6" min={1} max={6} />
                            <p className="text-pink-600 text-left">Gewichtung in % <span className="font-light text-xs text-pink-500">(zB. 60%)</span></p>
                            <div className="flex justify-center items-center">
                                <Input type="number" value={weight} onChange={(e) => setWeight(parseFloat(e.target.value))} placeholder="80" />
                                <p className="text-pink-600 text-xl ml-4">%</p>
                            </div>
                            <DialogClose asChild>
                                <button onClick={() => createGrade(gradeName, gradeValue, weight)} className="p-2 bg-pink-600 mt-4 text-white rounded-xl">Note hinzufügen</button>
                            </DialogClose>
                        </DialogDescription>
                        </DialogHeader>
                    </DialogContent>
                ...
                {subject.subject.grades.map((grade: any) => (
                    <div 
                        ...
                        className={`p-8 ${isDeleteMode ? "bg-pink-300" : "bg-pink-600"} ${isDeleteMode && 'animate-pulse'} bg-pink-600 flex items-center text-white text-xl font-bold justify-between rounded-3xl w-[100%] h-16 border-white border-2 shadow-lg`} key={grade.name}
                    >
